...
(function (global, factory) {
	...
	(factory((global.ylfCharts = {}),global.echarts));
 ...
 * 通用字体
 */
var commonFontFamily = 'Microsoft YaHei';

/**
 * 默认字体大小
 * @type {String}
 */
var commonFontSize = '12';

/**
 * y轴分割线颜色
 */
var yAxisSplitColor = '#e8e8e8';

/**
 * [axislineColor axisline颜色]
 * @type {String}
 */
var axislineColor = 'rgba(0, 0, 0, 0.45)';

/**
 * 图例颜色
 */
var legendColor = '#00d8f1';

/**
 * [legendIconWidth 图例小图标宽]
 */
var legendIconWidth = 16;

/**
 * [legendIconHeight 图例小图标高]
 */
var legendIconHeight = 3;

/**
 * 坐标轴label颜色码
 */
var axisLabelColor = '#525e71';
...
var EMPTY_DATA = { category: [], xAxis: [], series: [] };
 ...
 */
var getCategoryCollection = function (chain) {
 ...
 * @return {Array}
 */
var getLegendCollection = function (group) {
    var legendData = group.map(function (g) {
        return {
            name: g
        }
    });
 ...
 * @return {Boolean}
 */
var hasConsensusxAxis = function (data) {
    var hasXAsix = false;
 ...
 * @return {Boolean}
 */
var hasConsensusxAxisBase = function (data) {
    var hasXAsix = false;
    data.forEach(function (it) {
        if (it['xAxis']) {
            // 与后台约定，如果有xAxis  x轴data就取xAxis的集合
 ...
 * @return {Array}
 */
var getxAxisName = function (chain, hasAxis) {
    var tmpArr;
    var xAxisArr = [];
    if (hasAxis) {
        tmpArr = chain.filter(function (it) {
 ...
 * @return {String}
 */
var lookGroupValue = function (groupName, xAxis$$1, chains) {
    var val = chains
        .filter(function (it) {
            return (
                it.name === xAxis$$1 &&
 ...
 *
 */
var createSeriesCollection = function (ref) {
    var group = ref.group;
    var data = ref.data;
    var xAxis$$1 = ref.xAxis;
    var chain = ref.chain;
    var hasxAxis = ref.hasxAxis;
    var name = ref.name;
    ...
    group.forEach(function (g) {
        var seriesTemp = [];
        if (hasxAxis) {
            seriesTemp = xAxis$$1.map(function (axis) {
                var val = lookGroupValue(g, axis, chain);
                var obj = g;
            ...
            seriesTemp = data.map(function (d) {
                if (d.category === g) {
                    return {
                        name: d.name,
                        value: d.value,
        ...
        seriesCollect.push({
            type: name,
            name: g,
            data: seriesTemp.filter(function (it) {
                return it !== undefined
            })
        });
    });
 ...
 */
var getGroupSeriesObj = function (originData, name) {
    // 非空判断处理
    if (isEmptyData(originData)) {
        return EMPTY_DATA
    }
    // 拷贝一份原始数据
    var data = deepCopy(originData) || [];

    // 判断是否存在xAxis共识
    var hasxAxis = hasConsensusxAxis(data);

    // 转换成lodash chains
    var chain = lodash.chain(data);
    // 取分类数据集合
    var group = getCategoryCollection(chain);

    // 取xAxis 轴分组集合
    var xAxis$$1 = getxAxisName(chain, hasxAxis);

    // 构建series对象集合
    var series = createSeriesCollection({
        group: group,
        data: data,
        xAxis: xAxis$$1,
        chain: chain,
 ...
 * @return
 */
var getNormalSeriesObj = function (originData, __chartName__) {
    var category = [];
    var datas = [];
    var data = deepCopy(originData);
    var hasxAxis = hasConsensusxAxisBase(data);
    ...
    function hasDataVal(xAxis$$1, data) {
        var v = '';
        data.forEach(function (d) {
            if (d.name === xAxis$$1) {
                v = d;
    ...
    if (Array.isArray(data)) {
        if (hasxAxis) {
            datas = xAxisArr.map(function (xAxis$$1) {
                category.push(xAxis$$1);
                var it = hasDataVal(xAxis$$1, data);
                var obj = {
                    name: xAxis$$1,
                    value: it !== '' ? it.value : 0,
                    dataObj: it !== '' ? it : '',
...
var getCategory = function (originData) {
    // 转换成lodash chains
    var chain = lodash.chain(originData);
    // 取分类数据集合
    var group = getCategoryCollection(chain);
...
function getProps(op, modal, key) {
    var mark = function (prop, m, k) {
        var opt = propsCheck(prop, m);
        var v = opt[m][k] !== undefined ? opt[m][k] : '';
        return v
    };
 ...
 * @param key 属性名
 */
function getSpecialProps(op, key) {
    var prop = getProps(op, 'specialProp', key);
    return prop
}
 ...
 * @param key 属性名
 */
function getAxisProps(op, key) {
    var prop = getProps(op, 'axisProps', key);
    return prop
}
 ...
 */
function getAxisPropsGroup(prop, keys) {
    var obj = {};
 ...
 * @param key 属性名
 */
function getCommonProps(op, key) {
    var prop = getProps(op, 'commonProp', key);
    return prop
}
 ...
 * @param {Array} keys
 */
function getLegendProps(prop, keys) {
    var k = 'commonProp';
 ...
 */
function getAxisLabel(op, type) {
    if (!type || type === '') {
        type = 'xAxis';
    }
    var valueDisplay = getAxisProps(op, 'axisDisplay');
    var axisColor = getAxisProps(op, 'axisColor');
    var obj = {
        textStyle: {
            color: axisColor || axisLabelColor,
    ...
    var interval = getAxisProps(op, 'interval');
    if (rotate === '1') {
        obj.rotate = 0;
    } else if (rotate === '2') {
        obj.rotate = angle;
...
function getAxisLine(op, type) {
    // 直连点样式需求。折线不显示轴线。柱图显示轴线。为了兼容旧产品。做此判断
    var isLine = getAxisProps(op, 'isShowAxisLine');
    var obj;
    if (!isLine) {
        if (type === 'bar') {
            obj = false;
...
function getAxisName(props, name, unit) {
...
var getMaxMin = function (data) {
    ...
    if (max > 10 && max <= 100) {
        max = lodash.ceil(max, -1);
    } else if (max > 100 && max <= 1000) {
        max = lodash.ceil(max, -2);
    } else if (max > 1000) {
        max = lodash.ceil(max, -3);
    } else if (max <= 1) {
        max = lodash.ceil(max, 1);
...
var getMaxMinByDb = function (series) {
    var lData = [];
    var rData = [];
    ...
    var lmaxmin = getMaxMin(lData);
    var rmaxmin = getMaxMin(rData);
    return {
        lmax: lmaxmin.max,
        lmin: lmaxmin.min,
        rmax: rmaxmin.max,
 ...
 * @param param {max , min }
 */
function axisMaxmin(data, props, _type, dbObj) {
    var type = _type || getAxisProps(props, 'maxminType');
    var getylMax = getAxisProps(props, 'ylMax');
    var getylMin = getAxisProps(props, 'ylMin');
    var getyrMax = getAxisProps(props, 'yrMax');
    var getyrMin = getAxisProps(props, 'yrMin');
    var maxmin = {
        max: 100,
        min: 0
    };
...
// 改变横纵坐标顺序
function changeAxis(option) {
    var axisX = lodash.cloneDeep(option.xAxis);
    var axisY = lodash.cloneDeep(option.yAxis);
 ...
 * Created by yelingfeng on 2016/8/16.
 */
function getGirdOption(props) {
    var ref = getAxisPropsGroup(
        props,
        ['gridleft', 'gridRight', 'gridBottom', 'gridTop']
    );
    var gridleft = ref.gridleft;
    var gridRight = ref.gridRight;
    var gridBottom = ref.gridBottom;
    var gridTop = ref.gridTop;
    var grid = {
        left: gridleft || 10,
        right: gridRight || 10,
...
// 折线图tooltip划过处理 全局变量
var LINE_HOVER_KEY = '__ECHART_LINE_HOVERDATA_X_';

// 属性标题位置类型
var PROP_TITLE_ALIGN = {
    TOPLEFT: '1',
    TOPCENTER: '2',
    TOPRIGHT: '3',
    BOTTOMCENTER: '4',
 ...
 * @returns {*}
 */
function getLegend(data, props, type) {
    var legend = {};
    var lp = getLegendProps(props, [
        'isLegend',
        'legendPosition',
        'orientType',
        'legendLeft',
...
function handlerHoverGData(params) {
    ...
    if (lodash.isArray(params)) {
        for (var i = 0; i <= params.length; i++) {
            var it = params[i];
            if (lodash.isObject(it.data.dataObj)) {
                d = it.data;
            } else if (lodash.isString(it.data.dataObj)) {
                d = it.data;
            }
            break
        }
...
function handlerInfoCon(type, infoArr, flag) {
...
function itemTooltip(params, props) {
    var _content;
    var infoArr = props.toolTipProp.infoCon ||[];
    var per = props.toolTipProp.replace || '%%PERCENT%%';
    if (params.data) {
        var dataObj = params.data.dataObj ? params.data.dataObj : params.data;
    ...
    if (_content === undefined || _content === '') {
        if (isNaN(params.value)) {
            ...
            _content = "" + (params.name) + (handlerInfoCon('name', infoArr)) + (params.value) + (handlerInfoCon('value', infoArr));
        }
    }
    return _content
}
...
function axisTooltip(params, unitObj, props) {
    ...
    if(infoArr.length ){
        ...
        if (params.length === 1 && params[0].data.group === 'noGroup') {
            var data = params[0].data.dataObj;
            _content = data.info ?
                ("" + (data.info) + (handlerInfoCon('info', infoArr))) :
                ("" + (data.name) + (handlerInfoCon('name', infoArr)) + (data.value) + (handlerInfoCon('value', infoArr)));
        } else {
            // 多数据
            _content = (params[0].name) + "<br>";
            var dataArr = sequenceData(params, props);
...
// 字体设置
function getFontStyle(fontSize, align) {
...
function getAxisPointer(
    ...
    if ( type === void 0 ) type = 'shadow';
    if ( shadowStyle === void 0 ) shadowStyle = 'rgba(1, 91, 143, 0.4)';
...
// padding
function getPadding(pade) {
    if ( pade === void 0 ) pade = 9;

    return pade
}
// tooltip边框
function tooltipBox(type, content) {
...
// tip 通用格式
function getTooltip(config, props) {
    if ( config === void 0 ) config = {};
    ...
    if(props && props.toolTipProp){
        ...
        var tipObj = {
            ...
            formatter: function(params) {
                var tooltips = callBackTooltip(
                    params,
                    trigger,
                    { ylUnit: ylUnit, yrUnit: yrUnit },
                    props
                );
 ...
 * [class基类]
 */
var BasicClass = function BasicClass(op) {
    this.options = op;
    this._initEC();
};
BasicClass.prototype._initEC = function _initEC () {
        var this$1 = this;
    ...
    this.EC.on('click', function () {
            var ref;

            var args = [], len = arguments.length;
            while ( len-- ) args[ len ] = arguments[ len ];
        (ref = this$1).clickHandler.apply(ref, args);
    });
};

BasicClass.prototype.clickHandler = function clickHandler () {
        var ref;

        var arg = [], len = arguments.length;
        while ( len-- ) arg[ len ] = arguments[ len ];
    if(isFunction(this.options.onClick)){ (ref = this.options).onClick.apply(ref, arg); }
};

BasicClass.prototype.render = function render (data) {
    if (this.EC && data) {
        this.EC.clear();
        this.originData =
...
BasicClass.prototype.updateProp = function updateProp (props) {
    this.options.props = props;
    this.create();
};
BasicClass.prototype.build = function build (option) {
    if (this.EC) {
        this.EC.clear();
...
BasicClass.prototype.resize = function resize () {
    if (this.EC) {
        this.EC.resize();
    }
};
BasicClass.prototype.getECId = function getECId () {
    return this.EC.id
};
BasicClass.prototype.hideTip = function hideTip () {
    this.EC.dispatchAction({
        type: 'hideTip'
    });
};
BasicClass.prototype.showLoading = function showLoading () {
    this.EC.showLoading('', {
        text: 'loading',
        color: '#00ccff',
        textColor: '#fff',
...
BasicClass.prototype.hideLoading = function hideLoading () {
    this.EC.hideLoading();
};
// 取legend
BasicClass.prototype.getLegendModel = function getLegendModel (category, props) {
    return getLegend(category, props)
};
// 取tooltip
BasicClass.prototype.getTooltipModel = function getTooltipModel (config, props) {
    return getTooltip(config, props)
};
// 取colorlist 和 axisColor
BasicClass.prototype.getColorConfig = function getColorConfig (props) {
    return {
        color: getCommonProps(props, 'colorList') || commonColorList,
        nameColor: getAxisProps(props, 'axisColor')
    }
};
// 取grid
BasicClass.prototype.getGirdOptionModel = function getGirdOptionModel (props) {
    return getGirdOption(props)
};
// 取textStyle
BasicClass.prototype.getTextStyleModel = function getTextStyleModel () {
    return {
        fontFamily: commonFontFamily,
 ...
 * @return {Object}
 */
BasicClass.prototype.getChartsSettings = function getChartsSettings (series, trigger) {
    var category = getCategory(this.originData);
    var props = deepCopy(this.options.props);
    var ref = this.getColorConfig(props);
        var color = ref.color;
    var grid = this.getGirdOptionModel(props);
    var legend = this.getLegendModel(category, props);
    var tooltip = this.getTooltipModel(
        { trigger: trigger || 'axis' },
        props
    );
    var textStyle = this.getTextStyleModel();
    return {
        color: color,
        grid: grid,
        legend: legend,
 ...
 * 坐标系图表 基类
 */
var AxisChart = (function (BasicClass$$1) {
    function AxisChart () {
        BasicClass$$1.apply(this, arguments);
    }

    if ( BasicClass$$1 ) AxisChart.__proto__ = BasicClass$$1;
    AxisChart.prototype = Object.create( BasicClass$$1 && BasicClass$$1.prototype );
    AxisChart.prototype.constructor = AxisChart;

    AxisChart.prototype.render = function render (d) {
        BasicClass$$1.prototype.render.call(this, d);
    };
    // 取axisLine
    AxisChart.prototype.getAxisLineModel = function getAxisLineModel (chartName, props) {
     ...
     * @return {[type]} [description]
     */
    AxisChart.prototype.singleChartSetting = function singleChartSetting () {
        var ref = getNormalSeriesObj(
            this.originData,
            this.__chartName__
        );
        var seriesObj = ref.seriesObj;
        var category = ref.category;
        var axisSet = this.getAxisSettings(category);
        var chartSet = this.getChartsSettings([seriesObj]);
     ...
     * @return {[type]} [description]
     */
    AxisChart.prototype.groupChartSetting = function groupChartSetting () {
        var ref = getGroupSeriesObj(
            this.originData,
            this.__chartName__
        );
        var xAxis = ref.xAxis;
        var series = ref.series;
        var axisSet = this.getAxisSettings(xAxis);
        var chartSet = this.getChartsSettings(series);
        return Object.assign(chartSet, axisSet)
    };
     ...
     * @return {Object}
     */
    AxisChart.prototype.getAxisSettings = function getAxisSettings (xAxisData) {
        var props = lodash.cloneDeep(this.options.props);
        var ref = this.getColorConfig(props);
        var nameColor = ref.nameColor;
        var aLine = this.getAxisLineModel(this.__chartName__, props);
        var xAxis = this.getxAxisConfig(xAxisData, aLine, nameColor);
        var yAxis = this.getyAxisConfig(this.originData, aLine, nameColor);
        return {
            xAxis: xAxis,
            yAxis: yAxis
        }
     ...
     * @return {Array}
     */
    AxisChart.prototype.getxAxisConfig = function getxAxisConfig (category, axisLine, nameColor) {
        var config = [];
        var props = this.options.props;
        var name = getAxisName(props, 'xName', 'xUnit');
        var axisLabel = getAxisLabel(props, 'xAxis');
        config.push({
            name: name,
            type: 'category',
            nameTextStyle: {
     ...
     * @return  {Array}
     */
    AxisChart.prototype.getyAxisConfig = function getyAxisConfig (data, axisLine, nameColor) {
        var config = [];
        var props = this.options.props;
        var name = getAxisName(props, 'ylName', 'ylUnit');
        this.maxMin = axisMaxmin(data, props, 0, { isAxis: true });
        var axisLabel = getAxisLabel(props, 'yAxis');
        var splitLine = getSplitLine();
        var aLine;
        if (axisLine && lodash.isObject(axisLine)) {
            aLine = lodash.clone(axisLine);
            aLine.show = false;
        }
        config.push({
    ...
    /** 复制一个Axis，一般用于多y轴**/
    AxisChart.prototype.cloneAxis = function cloneAxis (axis) {
        var Axis = lodash.cloneDeep(axis);
...
var Bar = (function (AxisChart$$1) {
    ...
    if ( AxisChart$$1 ) Bar.__proto__ = AxisChart$$1;
    Bar.prototype = Object.create( AxisChart$$1 && AxisChart$$1.prototype );
    Bar.prototype.constructor = Bar;
    Bar.prototype.initBar = function initBar () {
        this.__chartName__ = 'bar';
    };
    Bar.prototype.render = function render (data) {
 ...
 */
var BaseBar = (function (Bar$$1) {
    function BaseBar(op) {
        Bar$$1.call(this, op);
        this.name = 'BaseBar';
    }

    if ( Bar$$1 ) BaseBar.__proto__ = Bar$$1;
 ...
 * Created by zhao on 2017/12/18.
 */
var GroupBar = (function (Bar$$1) {
    function GroupBar(op) {
        Bar$$1.call(this, op);
        this.name = 'GroupBar';
    }

    if ( Bar$$1 ) GroupBar.__proto__ = Bar$$1;
 ...
 * Created by zhao on 2017/12/18.
 */
var LevelBar = (function (Bar$$1) {
    function LevelBar(op) {
        Bar$$1.call(this, op);
        this.name = 'LevelBar';
    }

    if ( Bar$$1 ) LevelBar.__proto__ = Bar$$1;
    ...
    LevelBar.prototype.orderNumSort = function orderNumSort (option) {
        var data = option.series[0].data;
        var y = option.yAxis[0].data;
        var result = [];
        data.forEach(function (item) {
            var i = item.dataObj.ordernum;
            result[i - 1] = item;
        });
        // 判断是否已经为横向
        if (y) {
 ...
 */
var TYPE_LINE = 'line';
var MultiBar = (function (Bar$$1) {
    function MultiBar(op) {
        Bar$$1.call(this, op);
        this.name = 'MultiBar';
    }

    if ( Bar$$1 ) MultiBar.__proto__ = Bar$$1;
